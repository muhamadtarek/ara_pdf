...
fn extract_expected_text() {
    let docs = vec![
        expected!("Interview questions (1).pdf", "Disclaimer"),
        expected!("Mohamed_Tarek (1).pdf", "Mohamed Tarek Mehany"),
    ];
    for doc in docs {
        doc.test();
    }
...
#[test]
// Iterate over all PDFs in the `tests/docs` directory
fn extract_all_docs() {
    let docs = std::fs::read_dir("tests/docs").unwrap();
    for doc in docs {
        let doc = doc.unwrap();
